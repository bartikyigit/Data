...
# Pandas Sütun İsimlerini Güvence Altına Al (Case Sensitivity Hatasını Önler)
raw_age_data.columns = raw_age_data.columns.str.lower()
...
with c2:
    if camp_options:
        ...
        camp_dates = raw_age_data[raw_age_data['camp_id'] == sel_camp_id]['tarih'] if 'tarih' in raw_age_data.columns else pd.Series(dtype='object')
...
# Veri Kontrolü ve Index Kalkanı
if camp_data is None or camp_data.empty:
    st.warning("Hesaplanabilir geçerli veri bulunamadı (dakikası 0 olan veya filtrelenmiş eksik veriler).")
    st.stop()

camp_data.columns = camp_data.columns.str.lower() # Garantiye al
if 'tarih' not in camp_data.columns:
    camp_data = camp_data.reset_index()
    camp_data.columns = camp_data.columns.str.lower()

if 'impact_score' not in camp_data.columns:
    st.error("Yüklenen verilerde Hız, Mesafe gibi Impact Skorunu hesaplayacak temel metrikler eksik!")
    st.stop()
...
with tab1:
    ...
    if not unique_dates:
        ...
        with col_a:
            ...
            # Sadece var olan sütunları tabloya ekle
            display_cols = ['player_name', 'impact_score', 'status_tag']
            if 'dist_25_plus' in day_data.columns: display_cols.append('dist_25_plus')
            if 'player_load' in day_data.columns: display_cols.append('player_load')
            if 'smax_kmh' in day_data.columns: display_cols.append('smax_kmh')
                
            show_df = day_data[display_cols].copy()
            
            # Dinamik isimlendirme
            col_renames = ['OYUNCU', 'IMPACT SKOR', 'İSTATİSTİKSEL DURUM']
            if 'dist_25_plus' in day_data.columns: col_renames.append(d_dist_25)
            if 'player_load' in day_data.columns: col_renames.append(d_load)
            if 'smax_kmh' in day_data.columns: col_renames.append(d_smax)
            
            show_df.columns = col_renames
            show_df.index = np.arange(1, len(show_df) + 1)
            
            format_dict = {'IMPACT SKOR': '{:.1f}'}
            if d_dist_25 in show_df.columns: format_dict[d_dist_25] = '{:.0f}'
            if d_load in show_df.columns: format_dict[d_load] = '{:.1f}'
            if d_smax in show_df.columns: format_dict[d_smax] = '{:.1f}'
            
            st.dataframe(show_df.style.format(format_dict).background_gradient(cmap='Greens', subset=['IMPACT SKOR'], vmin=20, vmax=90), 
...
with tab2:
    ...
    # DİNAMİK AGGREGATION (Sadece Var Olan Sütunlar İçin)
    agg_dict = {'avg_impact': ('impact_score', 'mean')}
    
    if 'dist_25_plus_pm' in camp_data.columns:
        agg_dict['avg_high_speed'] = ('dist_25_plus_pm', 'mean')
    if 'player_load_pm' in camp_data.columns:
        agg_dict['avg_load'] = ('player_load_pm', 'mean')
    if 'smax_kmh' in camp_data.columns:
        agg_dict['max_speed'] = ('smax_kmh', 'max')
        
    agg_dict['session_count'] = ('tarih', 'count') if 'tarih' in camp_data.columns else ('player_name', 'count')
    
    camp_impact = camp_data.groupby('player_name').agg(**agg_dict).reset_index().sort_values('avg_impact', ascending=False)
    camp_impact.index = np.arange(1, len(camp_impact) + 1)
    
    # Dinamik Kolon İsimlendirme
    col_names = ['OYUNCU', 'HAFTA ORT. İMPACT']
    format_dict = {'HAFTA ORT. İMPACT': '{:.1f}'}
    
    if 'avg_high_speed' in camp_impact.columns:
        col_names.append(f'ORT. {d_dist_25} (m/dk)')
        format_dict[f'ORT. {d_dist_25} (m/dk)'] = '{:.2f}'
    if 'avg_load' in camp_impact.columns:
        col_names.append(f'ORT. {d_load} (/dk)')
        format_dict[f'ORT. {d_load} (/dk)'] = '{:.2f}'
    if 'max_speed' in camp_impact.columns:
        col_names.append(f'HAFTA {d_smax}')
        format_dict[f'HAFTA {d_smax}'] = '{:.1f}'
        
    col_names.append('KATILDIĞI SEANS')
    camp_impact.columns = col_names
    
    # Çizim
    styler = camp_impact.style.format(format_dict).background_gradient(cmap='Greys', subset=['HAFTA ORT. İMPACT'], vmin=40, vmax=80)
    if f'HAFTA {d_smax}' in camp_impact.columns:
        styler = styler.highlight_max(subset=[f'HAFTA {d_smax}'], color='#dcfce7')
...
with tab3:
    ...
    if 'tarih' in raw_age_data.columns:
    ...
    if historical_raw.empty:
        ...
        if historical_processed is not None and not historical_processed.empty:
            historical_processed.columns = historical_processed.columns.str.lower()
            if 'tarih' not in historical_processed.columns:
                historical_processed = historical_processed.reset_index()
                
            if sel_player not in historical_processed['player_name'].values:
...
with tab4:
    ...
    if 'tarih' in camp_data.columns:
        ...
        if len(player_trend_data) >= 2:
            ...
            daily_impact['tarih_str'] = pd.to_datetime(daily_impact['tarih']).dt.strftime('%d.%m')
        ...
        st.warning("Trend analizi için tarih verisi bulunamadı.")
